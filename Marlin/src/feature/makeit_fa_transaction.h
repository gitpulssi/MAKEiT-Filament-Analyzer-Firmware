 ...
 * the non-blocking evaluated test point. Results intentionally do not survive
 * a controller reset because reset also invalidates the physical feed context.
 */
#pragma once
...
public:
  ...
  /**
   * Execute one point exactly once for the currently retained point ID.
   *
   * Repeating the same point ID with the same normalized parameter hash is a
   * replay and never starts extrusion again. Reusing the same point ID with a
   * different hash is rejected as a conflict.
   */
