 ...
 *   - mid-point feed-efficiency abort
...
public:
   ...
   */
  static bool run_segmented_feed_test(float total_mm, float feed_mm_min, float segment_mm, uint8_t max_inflight, uint16_t report_ms);

  /**
   * Start one evaluated extrusion point.
   *
   * The hotend must already be at a non-zero target and within temp_tolerance
   * of that target. M873 resets the encoder, runs the same segmented motion
   * engine, samples temperature and heater power, then reports PASS,
   * LOW_FEED, INVALID_TEMP, or ERROR when the point ends.
   */
